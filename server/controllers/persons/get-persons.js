...
module.exports.getUnapprovedResources = {
    handler: function(request, reply) {
            ...
            if (!request.params.id) {
                ...
                connection.query(`SELECT * from persons where unapproved = 0 AND id = ?`, [request.params.id], function(err, rows, fields) {
                    if (err) { return reply(Boom.badRequest(`Error getting all from persons`)); }
            ...
            if (!request.params.id) {
                connection.query(`SELECT * from persons where approved = 1`, function(err, rows, fields) {
                    ...
                    */
                    ...
                    if (resources.length <= 0) {
                        return reply(Boom.badRequest("nothing to return for persons"));
                    } else {
                        return reply(resources);
                    }
                });
                ...
                connection.query(`SELECT * from persons where unapproved = 1 AND id = ?`, [request.params.id], function(err, rows, fields) {
                    if (err) { return reply(Boom.badRequest(`Error getting all from persons`)); }
                    var resource = rowsToJS(rows);
...
module.exports.getApprovedByType = {
    handler: function(request, reply) {
            ...
            if (!request.params.id) {
                ...
                connection.query(`SELECT * from persons where unapproved = 1 AND id = ? AND type = ?`, [request.params.id, request.params.type], function(err, rows, fields) {
                    if (err) { return reply(Boom.badRequest(`Error getting all from persons`)); }
                    var resource = rowsToJS(rows);

                    if (resource.length <= 0) {
                        return reply(Boom.badRequest(`Either persons is not approved, type DNE or id DNE`));
                    } else {
                        return reply(resource);
                    }
