...
module.exports.postQuiz = {
    handler: function(request, reply) {
            ...
            //cross-referenced with organizations' attributes

            //3. A dictionary will be created that maps the organizationID -> number of matches from step 2)

            //4. After all organizations have been checked, loop thru dict (from step 4), return top 5 matches
            ...
            //select * approved organizations, go through rows, parse categories, topics, etc
            //into temporary arrays which will be cross-referenced with quiz_categories, etc.

            var query = connection.query(`SELECT 
			id, clothing, ethnicities, instruments, categories, shape 
			FROM organizations WHERE approved = 1`, (err, rows, fields) => {
                if (err) { return reply(Boom.badRequest("error getting organizations in post-quiz-organization")); }
                if (rows.length <= 0) { return reply(Boom.badRequest("There are no approved organizations")); }
                var jsobj = rowsToJS(rows);

                var tempArray = [];
                var toCheck = ["clothing", "ethnicities", "instruments", "categories", "shape"];
                for (var i in jsobj) { //loop thru every organization
                    for (var whichCol in toCheck) { //loop through every column (see the array 2 lines above...)
                        ...
                        } //now we are done looping thru a certain column for a single organization
                        //tempArray = [];
                ...
                } //end looping thru every organization
                ...
                var query = connection.query(`SELECT * FROM organizations WHERE id in ?`, [
                    [toReturn]
                ], (err, rows, fields) => {
