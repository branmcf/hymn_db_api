  ...
  handler: function (request, reply) {
    ...
    if(!request.params.id) {
      ...
      if (request.params.id) {
          ...
          connection.query(`SELECT * from resources where id = ${request.params.id}`, function(err, rows, fields) {
          if (err) { return reply(Boom.badRequest()); }

            var JSObj = rowsToJS(rows[0]);
            //var JSObj = rows;
          ...
          if(JSObj.approved == false || JSObj.approved == 0) {
            ...
            resourceData = {
              ...
              url:            JSObj.website,
              author:         JSObj.author,
              resource_date:  JSObj.resource_date,
              description:    JSObj.description,
              parent:         JSObj.parent,
              is_active:      JSObj.is_active,
              high_level:     JSObj.high_level,
              city:           JSObj.city,
              state:          JSObj.state,
              country:        JSObj.country,
              hymn_soc_member:JSObj.hymn_soc_member,
              is_free:        JSObj.is_free,
              user_id:        JSObj.user_id,
              user:           JSObj.user,
              pract_schol:    JSObj.pract_schol,
              approved:       JSObj.approved,
          /*
              languages:      JSObj.languages,
              ethnicities:    JSObj.ethnicities,
              ensembles:      JSObj.ensembles,
              categories:     JSObj.categories,
              accompaniment: 	JSObj.accompaniment,
              topics:         JSObj.topics,
              tags:           JSObj.tags,
              denominations:  JSObj.denominations,
              instruments:    JSObj.instruments
          */
              languages:      resLanguages[0],
              ethnicities:    resEth[0],
              ensembles:      resEnsembles[0],
              categories:     resCategories[0],
              accompaniment: 	resAcc[0],
              topics:         resTopics[0],
              tags:           resTags[0],
              denominations:  resDenominations[0],
              instruments:    resInstruments[0]
            ...
            */
            ...
            var theUrl = "/resource/" + String(JSObj.id);
